...
export function OnboardingWidget() {
  ...
  const completedSteps = hasIncompleteTour ? (existingTour.currentStep ?? 0) : 0;
        ...
        {!isExpanded ? (
          ...
          >
            <Button
              ...
              className="relative h-14 w-14 rounded-full bg-primary hover:bg-primary/90 shadow-md"
              size="sm"
            >
              <Sparkles className="h-6 w-6 text-white" />
              
              {/* Calm: no global ping animation (keeps hierarchy quiet). */}
              
              {/* Progress indicator for incomplete tours */}
              {hasIncompleteTour && (
                <div className="absolute -top-1 -right-1">
                  <div className="h-6 w-6 rounded-full bg-card border-2 border-ring flex items-center justify-center">
            ...
            {/* Tooltip */}
              ...
              <div className="bg-popover text-popover-foreground border border-border text-sm px-3 py-2 rounded-lg whitespace-nowrap shadow-lg">
                {hasIncompleteTour ? 'Continue configuration' : 'Begin profile configuration'}
                <div className="absolute top-full right-4 border-4 border-transparent border-t-border"></div>
              </div>
            </div>
          </motion.div>
        ) : (
          <motion.div
            ...
            className="bg-popover rounded-xl shadow-xl border border-border overflow-hidden"
          >
