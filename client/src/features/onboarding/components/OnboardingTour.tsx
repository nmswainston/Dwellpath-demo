...
import type { OnboardingStep as OnboardingStepModel, OnboardingTour as OnboardingTourModel } from '@shared/schema';

type OnboardingStep = OnboardingStepModel;
type OnboardingTour = OnboardingTourModel;
...
export function OnboardingTour({ isOpen, onClose, userProfile }: OnboardingTourProps) {
  ...
  const { data: existingTour } = useQuery<OnboardingTour | null>({
  ...
  const getStepIcon = (stepType: string) => {
    ...
    const iconClasses = `${iconSize} text-brand-text-light dark:text-primary-foreground`;
    const containerClasses = "relative flex items-center justify-center rounded-full";
    const backgroundClasses = "absolute inset-0 rounded-full bg-primary-foreground/8";
    const glowClasses = "absolute inset-0 rounded-full shadow-[0_0_6px_hsl(var(--primary-foreground)/0.12)] dark:shadow-[0_0_10px_hsl(var(--primary-foreground)/0.18)]";
        ...
        return (
          <div className="space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">{content.message}</h3>
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              {content.recommendations?.map((rec: Recommendation, index: number) => (
                <motion.div
                  key={rec.id}
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 mb-2 min-w-0">
                            <h4 className="text-sm font-semibold min-w-0 truncate">{rec.title}</h4>
                            ...
                            <div className="mt-2">
                              ...
                              <ul className="text-xs space-y-1">
                                {rec.actionItems.slice(0, 2).map((item, idx) => (
                                  <li key={idx} className="flex items-center space-x-1 min-w-0">
                                    <span className="w-1 h-1 bg-brand-bg-dark/50 rounded-full shrink-0"></span>
          ...
          <div className="space-y-4">
            ...
            {content.setupTasks && (
              <div className="space-y-2">
                ...
                {content.setupTasks.map((task: string, index: number) => (
                  <motion.div
                    key={index}
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
      ...
      >
        ...
        >
          ...
          <div className="px-4 pt-4 pb-0 mb-2">
            <span className="text-sm font-semibold text-primary-foreground/50 font-body tracking-wide">
